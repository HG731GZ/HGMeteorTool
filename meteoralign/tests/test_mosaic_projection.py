...
from meteoralign.alignment.constants import (
...
from meteoralign.application.app_mosaic import (
...
from meteoralign.mosaic.state import (
    MOSAIC_IMAGE_SIZE_MODE_MANUAL,
    MOSAIC_IMAGE_SIZE_MODE_PERCENT,
)
from meteoralign.mosaic_framing import MOSAIC_FRAMING_SCHEMA
from meteoralign.mosaic_common import (
    MOSAIC_PROJECTION_MODELS,
)
from meteoralign.mosaic_export import MosaicExportGeometry
from meteoralign.mosaic_model_io import MosaicCoverageCache
...
def test_clear_all_mosaic_models_requires_confirmation(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    monkeypatch.setattr(
        "meteoralign.application.app_mosaic.QMessageBox.question",
...
def test_quiet_mosaic_model_load_replaces_all_existing_models(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    for method_name in (
        ...
        setattr(window, method_name, lambda *args, **kwargs: None)
    monkeypatch.setattr("meteoralign.application.app_mosaic.QApplication.setOverrideCursor", lambda *_args: None)
    monkeypatch.setattr("meteoralign.application.app_mosaic.QApplication.restoreOverrideCursor", lambda: None)
...
def test_multi_model_import_filters_invalid_json_and_keeps_valid_items(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    for method_name in (
        ...
        setattr(window, method_name, lambda *args, **kwargs: None)
    monkeypatch.setattr("meteoralign.application.app_mosaic.QProgressDialog", _Progress)
    monkeypatch.setattr("meteoralign.application.app_mosaic.QApplication.setOverrideCursor", lambda *_args: None)
    monkeypatch.setattr("meteoralign.application.app_mosaic.QApplication.restoreOverrideCursor", lambda: None)
    monkeypatch.setattr("meteoralign.application.app_mosaic.QApplication.processEvents", lambda: None)
    monkeypatch.setattr(
        "meteoralign.application.app_mosaic.QMessageBox.warning",
